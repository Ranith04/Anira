...
export function Header() {
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'fixed inset-0 z-[150] bg-black/40 transition-opacity lg:hidden',
      ...
      <aside
        className={cn(
          'fixed inset-y-0 right-0 z-[160] w-[80%] max-w-sm bg-background-50 shadow-2xl transition-transform duration-300 lg:hidden',
          isMobileMenuOpen
      ...
      >
        ...
        <nav className="flex flex-col overflow-y-auto font-body pb-10">
          {NAV_LINKS.map((link) => (
          ...
          <Link
            to="/account"
            onClick={closeMobile}
            className="border-t border-background-200 px-5 py-4 text-base font-medium text-primary-500 hover:bg-background-100"
          >
            My Account
          </Link>
        </nav>
