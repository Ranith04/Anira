import { Search, Eye, Filter, Mail } from 'lucide-react'
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 font-body text-sm text-foreground-500">Manage your customer relationships</p>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-3xl border border-white/40 bg-white/60 shadow-xl backdrop-blur-xl">
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="p-12 text-center font-body text-foreground-500 animate-pulse">Loading customers...</div>
          ) : (
            <table className="w-full text-left">
              <thead className="bg-white/40">
                <tr className="border-b border-primary-500/10">
                  <th className="py-4 pl-6 font-body text-xs font-bold uppercase tracking-widest text-foreground-500">Customer</th>
                  <th className="py-4 font-body text-xs font-bold uppercase tracking-widest text-foreground-500">Orders</th>
                  <th className="py-4 font-body text-xs font-bold uppercase tracking-widest text-foreground-500">Total Spent</th>
              ...
              <tbody className="divide-y divide-primary-500/5 bg-transparent">
                {customers?.map((customer) => (
                  <tr key={customer.id} className="group transition-colors hover:bg-white/60">
                    <td className="py-4 pl-6">
                      <div className="flex items-center gap-4">
                        <div className="flex size-10 items-center justify-center rounded-full bg-primary-500/10 text-primary-600 font-heading font-bold shadow-sm border border-primary-500/20">
                          {customer.name.charAt(0)}
                        </div>
                        <div>
                          <p className="font-heading text-sm font-bold text-foreground-900 group-hover:text-primary-600 transition-colors">{customer.name}</p>
                          <p className="font-body text-xs text-foreground-500">{customer.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-4 font-body text-sm font-medium text-foreground-700">{customer.orders} orders</td>
                    <td className="py-4 font-heading text-sm font-bold text-foreground-900">{customer.spent}</td>
                    <td className="py-4">
                      <span
                        className={`inline-flex rounded-full px-2.5 py-1 font-body text-xs font-bold uppercase tracking-wider ${
                          customer.lastActive === 'Recent'
